...
public class DeleteItemServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String remove = request.getParameter("txtremove");
        ...
        try {
            ...
            for (String s : arr) {
                int rs = dao.deleteFood(Integer.parseInt(s));
                if (rs > 0) {
                    dao.insertManager(username, Integer.parseInt(s), 1);
                }
            }
        } catch (SQLException ex) {
            log("DeleteItemServlet_SQLException: " + ex.getMessage());
        } catch (NamingException ex) {
            log("DeleteItemServlet_NamingException: " + ex.getMessage());
        } finally {
